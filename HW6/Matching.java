	...
	public void remove() {// did not use.
		if (!canRemove) {
			throw new IllegalStateException(
					"Next method has not been called, or remove() is called multiple times in a row.");
		}
		if (previous == null) {
...
class cPair<T extends Comparable<T>, S extends Comparable<S>> implements Comparable<cPair<T, S>> {// comparable pair
																									// (tuple)
	public T first;
	public S second;
...
public class Matching {
	...
	private static void deleteString(String pattern) {
		...
		for (int i = 0; i < lines_cnt - 1; i++) {
			...
			for (int j = 0; j <= target.length() - 6; j++) {
				table.get(target.substring(j, j + 6)).delete(target.substring(j, j + 6),
						new cPair<Integer, Integer>(i + 1, j + 1));
			}
	...
	private static String delString(String target, List<Integer> indices) { // used to return new string
		...
		for (Integer idx : indices) { // similar to two pointer algorithm
			...
			stridx = idx + 6;// stridx may decrease, depending on the distance between indices, hence can
								// check all
		}
