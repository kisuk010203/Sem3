...
public class SortingTest
{
    ...
    static char DoSearch(int[] value){
		...
		for (int i = 0; i < value.length-1; i++) {
			if(decrease_count == -1 && value[i] > value[i+1]) decrease_count = value.length-i; // count number of max increasing consecutive subseq.
			if(increase_count == -1 && value[i] < value[i+1]) increase_count = i;
		...
		if(decrease_count < (int)Math.log(value.length)) // if the given array is already almost sorted, use insertionsort
			return 'I';
		else if(increase_count <= (int)Math.log(value.length)) // if the given array is already almost sorted backwards, use heapsort
			return 'H';
		if(max_term<100 && min_term> -100) return 'R'; // if values are sufficiently small, use radixsort
		for(int key : map.keySet()){
			square_time += map.get(key)*map.get(key);
		}
		if(square_time >= value.length * (int)Math.log(value.length) * 10) // if square_time is sufficiently large, use heapsort
			return 'H';
